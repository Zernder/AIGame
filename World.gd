...
func PlaceBarrier():
	if Input.is_action_just_pressed("PlaceBarrier") and Ally.Points >= 2 and BarrierCooldown == false:
...
func PlaceBeacon():
	if Input.is_action_just_pressed("PlaceBeacon") and Ally.Points >= 2 and BeaconCooldown == false:
		BeaconCooldown = true
